...
int getRecursiveHeightTreeNodes(TreeNode *root) {
	if (!root)
		return 0;
	int l = getRecursiveHeightTreeNodes(root->left);
	int r = getRecursiveHeightTreeNodes(root->right);

	return 1 + (l>r ? l: r);
}

/* Get the height of the tree iteratively */
int getIterativeHeightTreeNodes(TreeNode *root) {
	if (!root)
		return 0;
...
/* print values of a given level */
void printGivenLevel(TreeNode *root, int l) {
	if (!root)
		return;
...
/* level order iterative tree traversal */
void levelOrderIterativeTreeNodes(TreeNode * root) {

	int h = getRecursiveHeightTreeNodes(root);
//	int h2 = getIterativeHeightTreeNodes(root);
	for (int i=0; i<h; i++)
